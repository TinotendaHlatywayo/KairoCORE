...
<body class="bg-slate-900 text-slate-100 flex items-center justify-center min-h-screen">
    <div class="max-w-md w-full bg-slate-800 border border-slate-700 rounded-2xl p-8 shadow-2xl space-y-6">
        <div class="text-center space-y-2">
            ...
            <p class="text-xs text-slate-400">{{ __('Simulating payment gateway processing for school fee settlement.') }}</p>
        </div>
        ...
        <div class="space-y-3">
            ...
            <form action="{{ route('student.paynow.sandbox-complete') }}" method="POST">
                @csrf
            ...
            <a href="/student/my-fees" class="block text-center text-xs text-slate-400 hover:text-white pt-2">
                {{ __('Cancel and return to student portal') }}
            </a>
        </div>
    </div>
</body>
