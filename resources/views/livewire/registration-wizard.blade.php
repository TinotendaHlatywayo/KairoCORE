<div id="sc-reg-card" class="card shadow-sm border-0 p-4">

    {{-- ════════════════════════════════════════════════════════════════════
         WIZARD
         ════════════════════════════════════════════════════════════════════ --}}
    <div class="sc-step-appear">
        ...
        <div>
            ...
            <div class="row g-3">
                <div class="col-md-6 mb-3">
                    <label class="form-label fw-semibold text-secondary">{{ __('Registered Institution Name') }} <span class="text-danger">*</span></label>
                    <input type="text" wire:model.blur="schoolName" class="form-control @error('schoolName') is-invalid @enderror" placeholder="{{ __('e.g., Greenwood Academy') }}">
                ...
                <div class="col-md-6 mb-3" x-data="{
                    ...
                    <label class="form-label fw-semibold text-secondary">{{ __('Country') }} <span class="text-danger">*</span></label>
                    <div class="position-relative">
                        ...
                        <div x-show="open" @click.outside="open = false" x-transition
                             class="position-absolute w-100 mt-1 bg-white border rounded shadow-lg sc-country-list" style="z-index:40;">
                ...
                <div class="col-12 mb-3">
                    <label class="form-label fw-semibold text-secondary">{{ __('Physical Address') }} <span class="text-danger">*</span></label>
                    <textarea wire:model.blur="physicalAddress" rows="2" class="form-control @error('physicalAddress') is-invalid @enderror" placeholder="{{ __('Street address, city, region') }}"></textarea>
                    @error('physicalAddress') <div class="invalid-feedback">{{ $message }}</div> @enderror
                </div>

                <div class="col-md-6 mb-3">
                    <label class="form-label fw-semibold text-secondary">{{ __('System Language') }} <span class="text-danger">*</span></label>
                ...
                <div class="col-md-6 mb-3">
                    <label class="form-label fw-semibold text-secondary">{{ __('Primary Phone Number') }} <span class="text-danger">*</span></label>
                    <input type="text" wire:model.blur="phone" class="form-control @error('phone') is-invalid @enderror" placeholder="{{ __('+263771122334') }}">
                ...
                @if($institutionType === 'other')
                    <div class="col-md-6 mb-3">
                        <label class="form-label fw-semibold text-secondary">{{ __('Specify Other Institution Type') }} <span class="text-danger">*</span></label>
                        <input type="text" wire:model.blur="otherInstitutionType" class="form-control @error('otherInstitutionType') is-invalid @enderror" placeholder="{{ __('Specify type') }}">
                ...
                <div class="col-md-6 mb-3">
                    <label class="form-label fw-semibold text-secondary">{{ __('Administrator Full Name') }} <span class="text-danger">*</span></label>
                    <input type="text" wire:model.blur="adminName" class="form-control @error('adminName') is-invalid @enderror" placeholder="{{ __('Prof. Alex Mercer') }}">
                    @error('adminName') <div class="invalid-feedback">{{ $message }}</div> @enderror
                </div>

                <div class="col-md-6 mb-3">
                    <label class="form-label fw-semibold text-secondary">{{ __('Administrator Email Address') }} <span class="text-danger">*</span></label>
                    <input type="email" wire:model.blur="adminEmail" class="form-control @error('adminEmail') is-invalid @enderror" placeholder="{{ __('admin@greenwood.edu') }}">
                    @error('adminEmail') <div class="invalid-feedback">{{ $message }}</div> @enderror
                    <div class="form-text small">{{ __('Cannot be changed after registration for security. Username & password are set upon approval.') }}</div>
                </div>

                <div class="col-12 mb-3">
                    <label class="form-label fw-semibold text-secondary">{{ __('Institution Email Address') }} <span class="text-danger">*</span></label>
                    <input type="email" wire:model.blur="institutionEmail" class="form-control @error('institutionEmail') is-invalid @enderror" placeholder="{{ __('info@greenwood.edu') }}">
                    @error('institutionEmail') <div class="invalid-feedback">{{ $message }}</div> @enderror
                    <div class="form-text small">{{ __('Official institutional email for billing, legal correspondence, and public display. Must differ from administrator email.') }}</div>
                </div>
            </div>
        </div>
    @endif
    ...
    @if($currentStep === 2)
        <div>
            ...
            <div class="mb-3">
                <label class="form-label fw-semibold text-secondary">{{ __('Subdomain') }} <span class="text-danger">*</span></label>
                <div class="input-group">
                    <input type="text" wire:model.live.debounce.300ms="subdomain" class="form-control @error('subdomain') is-invalid @enderror" placeholder="{{ __('greenwood') }}">
                    <span class="input-group-text bg-light text-secondary">.{{ $this->baseDomain }}</span>
                </div>
                ...
                @if(!empty($subdomain) && !$errors->has('subdomain'))
                    @if($isSubdomainAvailable)
                        <div class="text-success small mt-1">{{ __('Available:') }} <strong>{{ $this->subdomainPreviewUrl }}</strong></div>
                    @else
            ...
            <div class="form-check form-switch bg-light p-3 rounded">
                <div class="ps-2">
                    ...
                    <p class="text-muted small mb-0">{{ __('Recommended. Seeds classes, test students, and mock assets for immediate testing.') }}</p>
                </div>
            </div>
        </div>
    @endif
    ...
    @if($currentStep === 3)
        @php
            $moduleIcons = [
                ...
                'exams' => '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M16.862 4.487l1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L6.832 19.82a4.5 4.5 0 0 1-1.897 1.13l-2.685.8.8-2.685a4.5 4.5 0 0 1 1.13-1.897L16.863 4.487Z"/><path d="M12 6.75h.01"/></svg>',
                ...
                'hr' => '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path fill-rule="evenodd" d="M7.5 5.25a3 3 0 0 1 3-3h3a3 3 0 0 1 3 3v.205c.933.085 1.857.197 2.774.334 1.454.219 2.476 1.483 2.476 2.916v2.415c0 1.477-.978 2.758-2.381 3.003a48.743 48.743 0 0 1-9.738 0 3.012 3.012 0 0 1-2.381-3.003V8.705c0-1.433 1.022-2.697 2.476-2.916A48.561 48.561 0 0 1 7.5 5.455V5.25Zm7.5 0v.09a49.27 49.27 0 0 0-6 0v-.09a1.5 1.5 0 0 1 1.5-1.5h3a1.5 1.5 0 0 1 1.5 1.5Z" clip-rule="evenodd"/></svg>',
                'boarding' => '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M15 21v-8a1.5 1.5 0 0 0-1.5-1.5H12A1.5 1.5 0 0 0 10.5 13v8m-6.75 0V9.45a3 3 0 0 1 .87-2.12l4.5-4.5A3 3 0 0 1 11.121 2h1.758a3 3 0 0 1 2.122.879l4.5 4.5a3 3 0 0 1 .879 2.121V21m-13.5 0h13.5"/></svg>',
                'clinic' => '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M21 8.25c0-2.485-2.099-4.5-4.688-4.5-1.935 0-3.597 1.126-4.312 2.733-.715-1.607-2.377-2.733-4.313-2.733C5.1 3.75 3 5.765 3 8.25c0 7.22 9 12 9 12s9-4.78 9-12Z"/></svg>',
                ...
                'lms' => '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z"/><path d="M15.91 11.672a.375.375 0 0 1 0 .656l-5.603 3.113a.375.375 0 0 1-.557-.328V8.887c0-.286.307-.466.557-.327l5.603 3.112Z"/></svg>',
        ...
        <div>
            <div class="d-flex flex-wrap align-items-center justify-content-between gap-2 mb-3">
                ...
                <div class="d-flex gap-2">
                    <button type="button" wire:click="selectAllModules" class="btn btn-sm" style="background: linear-gradient(135deg,#1e3a8a,#2563eb); color:#fff; border:none; border-radius:0.6rem; font-weight:700;">{{ __('Select All') }}</button>
            ...
            <div class="row g-2">
                @php
                    $moduleColors = [
                        'admissions' => ['bg' => '#e0e7ff', 'color' => '#4f46e5'],
                        'students' => ['bg' => '#dbeafe', 'color' => '#2563eb'],
                        'academics' => ['bg' => '#d1fae5', 'color' => '#059669'],
                        'exams' => ['bg' => '#fef3c7', 'color' => '#d97706'],
                        'attendance' => ['bg' => '#ccfbf1', 'color' => '#0d9488'],
                        'hr' => ['bg' => '#ede9fe', 'color' => '#7c3aed'],
                        'boarding' => ['bg' => '#ffe4e6', 'color' => '#e11d48'],
                        'clinic' => ['bg' => '#fee2e2', 'color' => '#dc2626'],
                        'library' => ['bg' => '#f3e8ff', 'color' => '#9333ea'],
                        'inventory' => ['bg' => '#ffedd5', 'color' => '#c2410c'],
                        'finance' => ['bg' => '#dcfce7', 'color' => '#16a34a'],
                        'communication' => ['bg' => '#e0f2fe', 'color' => '#0284c7'],
                        'website' => ['bg' => '#fae8ff', 'color' => '#c026d3'],
                        'lms' => ['bg' => '#cffafe', 'color' => '#0891b2'],
                        'knowledge' => ['bg' => '#fef9c3', 'color' => '#ca8a04'],
                        'reports' => ['bg' => '#f1f5f9', 'color' => '#475569'],
                        'administration' => ['bg' => '#e2e8f0', 'color' => '#334155'],
                        'saas' => ['bg' => '#d1fae5', 'color' => '#059669'],
                    ];
                @endphp
                @foreach($availableModules as $key => $mod)
                    @php 
                        $locked = in_array($key, $lockedModules, true);
                        $c = $moduleColors[$key] ?? ['bg' => '#f1f5f9', 'color' => '#475569'];
                    @endphp
                    <div class="col-12 col-md-6 col-lg-4 d-flex">
                        <label class="sc-mod-card {{ in_array($key, $selectedModules) ? 'selected' : '' }} {{ $locked ? 'locked' : '' }}" for="mod_{{ $key }}">
                            <div class="sc-mod-icon" style="background-color: {{ $c['bg'] }}; color: {{ $c['color'] }};">{!! $moduleIcons[$key] ?? '' !!}</div>
                            <div class="sc-mod-body">
                                <span class="sc-mod-name">
                                    {{ $mod['name'] }}
                                    @if($locked)
                                        <span class="sc-lock-badge"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" width="10" height="10"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>{{ __('Always On') }}</span>
                                    @endif
                                </span>
    ...
    <!-- Terms Acceptance + Navigation Buttons -->
    <div class="d-flex justify-content-between mt-4 border-top pt-3">
        @if($currentStep > 1)
            <button type="button" wire:click="prevStep" class="btn btn-outline-secondary" wire:loading.attr="disabled" wire:target="submit">{{ __('Back') }}</button>
        @else
            <div></div>
        @endif
    ...
    @if($currentStep === $totalSteps)
        ...
        <div class="form-check mt-3 pt-1">
            <input class="form-check-input @error('termsAccepted') is-invalid @enderror"
                   type="checkbox"
                   id="sc-wizard-terms"
                   wire:model.live="termsAccepted"
                   style="width: 1.15rem; height: 1.15rem; margin-top: 0.2rem; cursor: pointer; accent-color: #1e3a8a;">
            <label class="form-check-label text-secondary" for="sc-wizard-terms" style="font-size: 0.9rem; line-height: 1.55; cursor: pointer;">
                {{ __('I have read and agree to the') }}
                <a href="{{ route('platform.terms') }}" target="_blank" rel="noopener noreferrer" class="fw-semibold text-decoration-none">{{ __('Terms of Service') }}</a>
                {{ __('and') }}
                <a href="{{ route('platform.terms') }}" target="_blank" rel="noopener noreferrer" class="fw-semibold text-decoration-none">{{ __('Terms of Use') }}</a>.
            </label>
            @error('termsAccepted')
