                ...
                <div class="flex flex-wrap items-center gap-4">
                    ...
                    <div class="ml-auto flex items-center gap-2">
                        <span class="text-xs text-gray-400 hidden lg:inline">
                            @if($viewScope === 'stream')
                                {{ __('Click a lesson to edit or remove it.') }}
                            @else
                                {{ __('Drag between periods to reschedule instantly.') }}
                            @endif
                        </span>
                        @if($viewScope === 'stream')
                            @if($selectedCourseId)
                                ...
                                <tr class="hover:bg-gray-50/50 dark:hover:bg-gray-800/20 {{ $slot['is_break'] ? 'bg-amber-500/5 dark:bg-amber-500/5' : '' }}">
                                    ...
                                    @if($viewScope === 'stream')
                                        @foreach($days as $day)
                                            ...
                                            @endphp
                                            <td
                                                wire:key="streamslot_{{ $slot['id'] }}_{{ $day }}"
                                                class="p-2 border-r border-gray-200 dark:border-gray-800 align-top"
                                            >
                                                @if($slot['is_break'])
                                                ...
                                                @elseif(count($entries) > 0)
                                                    <div class="space-y-1.5">
                                                        @foreach($entries as $entry)
                                                            <div
                                                                wire:key="streamlesson_{{ $entry['lesson_id'] }}"
                                                                wire:click="openEditLessonModal({{ $entry['lesson_id'] }})"
                                                                class="p-2 rounded-lg border text-left shadow-sm cursor-pointer hover:scale-[1.01] transition-all {{ $entry['color_classes'] }}"
                                                            >
                                    ...
                                    @else
                                        @foreach($days as $day)
                                            ...
                                            >
                                                ...
                                                @else
                                                    <!-- Empty Slot → Click to add a lesson -->
                                                    <div
                                                        wire:click="openAddLessonModal({{ $slot['id'] }}, '{{ $day }}')"
                                                        class="text-xs text-gray-300 dark:text-gray-700 border border-dashed border-gray-200 dark:border-gray-800 rounded-lg py-4 cursor-pointer hover:border-green-400 dark:hover:border-green-500 hover:text-green-500 dark:hover:text-green-400 transition-colors"
                                                    >
                                                        + {{ __('Add Lesson') }}
                                                    </div>
                                                @endif
...
<!-- ADD LESSON MODAL -->
    @if($isAddModalOpen)
        <div class="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="add-modal-title" role="dialog" aria-modal="true">
            <div class="flex items-end justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
                <div class="fixed inset-0 transition-opacity bg-gray-500 bg-opacity-75" aria-hidden="true" wire:click="$set('isAddModalOpen', false)"></div>
                <span class="hidden sm:inline-block sm:align-middle sm:h-screen" aria-hidden="true">{{ __('&#8203;') }}</span>

                <div class="inline-block align-bottom bg-white dark:bg-gray-900 rounded-xl text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-width sm:w-full sm:max-w-lg border border-gray-200 dark:border-gray-800">
                    <div class="bg-white dark:bg-gray-900 px-6 pt-6 pb-4">
                        @php
                            $addSlot = \Modules\Timetables\Models\TimeSlot::find($addSlotId);
                        @endphp
                        <h3 class="text-lg font-bold text-gray-950 dark:text-white border-b border-gray-100 dark:border-gray-800 pb-3" id="add-modal-title">
                            {{ __('Add Scheduled Lesson') }} —
                            <span class="text-green-600 dark:text-green-400">{{ ucfirst($addDay) }} · {{ $addSlot?->name ?? 'Period' }}</span>
                        </h3>

                        <div class="mt-4 space-y-4">
                            <div>
                                <label class="block text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wide mb-1">{{ __('Subject') }}</label>
                                <select wire:model="addSubjectId" class="w-full text-sm rounded-lg border-gray-300 dark:bg-gray-800 dark:border-gray-700 dark:text-white">
                            ...
                            <div>
                                <label class="block text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wide mb-1">{{ __('Teacher') }}</label>
                                <select wire:model="addTeacherId" class="w-full text-sm rounded-lg border-gray-300 dark:bg-gray-800 dark:border-gray-700 dark:text-white">
                            ...
                            <div>
                                <label class="block text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wide mb-1">{{ __('Classroom') }}</label>
                                <select wire:model="addClassroomId" class="w-full text-sm rounded-lg border-gray-300 dark:bg-gray-800 dark:border-gray-700 dark:text-white">
                    ...
                    <div class="bg-gray-50 dark:bg-gray-800 px-6 py-4 flex justify-end gap-2 border-t border-gray-100 dark:border-gray-800">
                        <x-filament::button type="button" wire:click="$set('isAddModalOpen', false)" color="gray">
                            {{ __('Cancel') }}
                        </x-filament::button>
                        <x-filament::button type="button" wire:click="saveNewLesson" color="success">
                            {{ __('Add Lesson') }}
                        </x-filament::button>
                    </div>
                </div>
