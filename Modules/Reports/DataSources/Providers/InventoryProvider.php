...
class InventoryProvider extends AbstractDatasetProvider
{
    ...
    {
        return [
            $this->d('inventory.item', 'Inventory Items', 'inventory_items', [
                $this->f('sku', 'SKU Code'),
                $this->f('barcode', 'Barcode'),
                $this->f('name', 'Item Name'),
                $this->f('item_type', 'Item Type'),
                $this->f('unit_of_measure', 'UOM'),
                $this->f('reorder_level', 'Reorder Level', 'integer'),
                $this->f('current_quantity', 'Quantity on Hand', 'integer'),
                $this->f('average_unit_cost', 'Avg Unit Cost', 'currency'),
                $this->money('stock_value', 'Stock Value', '(inventory_item.current_quantity * inventory_item.average_unit_cost)'),
                $this->f('is_saleable', 'Saleable', 'boolean'),
                $this->f('sale_price', 'Sale Price', 'currency'),
                $this->f('category_name', 'Category', 'string', 'inventory_item_cat.name'),
                $this->f('low_stock', 'Low Stock Alert', 'boolean', 'CASE WHEN inventory_item.current_quantity <= inventory_item.reorder_level THEN 1 ELSE 0 END'),
            ], [
            ...
            $this->d('inventory.transaction', 'Inventory Movements', 'inventory_transactions', [
                $this->f('transaction_type', 'Transaction Type'),
                $this->f('quantity', 'Quantity', 'integer'),
                $this->f('balance_after', 'Balance After', 'integer'),
                $this->f('notes', 'Notes'),
                $this->f('created_at', 'Transaction Date', 'datetime'),
                $this->f('item_name', 'Item Name', 'string', 'inventory_transaction_item.name'),
                $this->f('item_sku', 'SKU', 'string', 'inventory_transaction_item.sku'),
            ], [
            ...
            $this->d('inventory.supplier', 'Suppliers', 'inventory_suppliers', [
                $this->f('name', 'Supplier Name'),
                $this->f('contact_person', 'Contact Person'),
                $this->f('phone', 'Phone'),
                $this->f('email', 'Email'),
                $this->f('tax_number', 'Tax Number'),
            ], [
            ...
            $this->d('assets.fixed_asset', 'Fixed Assets', 'fixed_assets', [
                $this->f('asset_number', 'Asset Number'),
                $this->f('serial_number', 'Serial Number'),
                $this->f('acquisition_date', 'Acquisition Date', 'date'),
                $this->money('purchase_cost', 'Purchase Cost', 'fixed_asset.purchase_cost'),
                $this->money('salvage_value', 'Salvage Value', 'fixed_asset.salvage_value'),
                $this->f('useful_life_years', 'Useful Life (yrs)', 'integer'),
                $this->f('depreciation_method', 'Depreciation Method'),
                $this->money('current_value', 'Current Value', 'fixed_asset.current_value'),
                $this->f('warranty_expiry', 'Warranty Expiry', 'date'),
                $this->f('status', 'Status'),
                $this->f('item_name', 'Item Name', 'string', 'fixed_asset_item.name'),
            ], [
            ...
            $this->d('assets.maintenance', 'Asset Maintenance Logs', 'asset_maintenance_logs', [
                $this->f('title', 'Title'),
                $this->f('type', 'Type'),
                $this->f('scheduled_date', 'Scheduled Date', 'date'),
                $this->f('completed_date', 'Completed Date', 'date'),
                $this->money('cost', 'Maintenance Cost', 'asset_maintenance.cost'),
                $this->f('status', 'Status'),
                $this->f('asset_number', 'Asset Number', 'string', 'asset_maintenance_asset.asset_number'),
            ], [
            ...
            $this->d('procurement.request', 'Procurement Requests', 'procurement_requests', [
                $this->f('request_number', 'Request Number'),
                $this->f('urgency', 'Urgency'),
                $this->f('status', 'Status'),
                $this->f('notes', 'Notes'),
                $this->f('created_at', 'Requested At', 'datetime'),
                $this->f('department_name', 'Department', 'string', 'procurement_request_dept.name'),
            ], [
            ...
            $this->d('procurement.order', 'Purchase Orders', 'procurement_orders', [
                $this->f('order_number', 'Order Number'),
                $this->f('order_date', 'Order Date', 'date'),
                $this->f('expected_delivery_date', 'Expected Delivery', 'date'),
                $this->f('status', 'Status'),
                $this->money('total_amount', 'Order Total', 'procurement_order.total_amount'),
                $this->f('supplier_name', 'Supplier', 'string', 'procurement_order_supplier.name'),
            ], [
            ...
            $this->d('procurement.item', 'Procurement Line Items', 'procurement_order_items', [
                $this->f('quantity_ordered', 'Quantity Ordered', 'integer'),
                $this->f('quantity_received', 'Quantity Received', 'integer'),
                $this->money('unit_cost', 'Unit Cost', 'procurement_item.unit_cost'),
                $this->f('order_number', 'Order Number', 'string', 'procurement_item_order.order_number'),
                $this->f('item_name', 'Item', 'string', 'procurement_item_inv.name'),
            ], [
